...
public class DebitAccountHandler {
    ...
    @Transactional
    public Account handle(DebitAccountCommand command) {
        AccountId accountId = AccountId.of(UUID.fromString(command.accountId()));
        Money money = Money.of(command.amount(), command.currency());
        Account account = accountDomainService.debit(accountId, money, command.description());
        eventPublisher.publishAccountDebited(account);
        return account;
    }
}